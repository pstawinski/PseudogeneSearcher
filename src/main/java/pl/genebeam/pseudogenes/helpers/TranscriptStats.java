...
public class TranscriptStats {
    ...
    private Multimap<Integer, SAMRecord> readsBySoftClipped = Multimaps.synchronizedSetMultimap(HashMultimap.create());

    public void addReadWithSmallInsertSize(Integer txIdx, SAMRecord read) {
        readsByIntronSize.put(txIdx, read);
    }

    public void addReadWithSoftClip(Integer txIdx, SAMRecord read) {
        readsBySoftClipped.put(txIdx, read);
    }
